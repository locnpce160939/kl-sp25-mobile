...
const BalanceDriverScreen = ({ navigation }) => {
  ...
  const fetchBalanceHistory = async () => {
    try {
      ...
      const response = await axios.get(
        `${BASE_URL}/api/balanceHistory/account`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
  ...
  const groupTransactionsByDate = (transactions) => {
    ...
    return Object.entries(groups)
      ...
      .sort(
        (a, b) =>
          new Date(b.data[0].transactionDate) -
          new Date(a.data[0].transactionDate)
      );
  };
  ...
  const BalanceHeader = () => (
    <View
      style={[styles.balanceContainer, { paddingTop: insets.top > 0 ? 0 : 16 }]}
    >
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      ...
      onPress={() =>
        navigation.navigate("BalanceHistoryDetail", {
